...
**100% client-side. No upload, no signup, no tracking.** Every roll runs in your browser. AI expansion is optional polish through a keyless multi-provider gateway; if it is down, the dice still roll.
...
Static Astro (`output: 'static'`) + React 19 islands + Tailwind v4 (`@tailwindcss/vite`). Shared atomic packages: `@chirag127/oz-ai` (AI, g4f failover), `@chirag127/oz-tokens-base` (token contract), `@chirag127/oz-chrome` (header/footer shell), `@chirag127/oz-file`. Randomness via Web Crypto with a `Math.random` fallback; share codec via `lz-string`. Zero heavy deps; AI libs load only when you trigger a feature.
